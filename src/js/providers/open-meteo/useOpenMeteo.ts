...
export const mapCurrent = (
  day: { temp_min: number; temp_max: number },
  current: OpenMeteoCurrent,
  lang: LanguageCode,
): CurrentData => {
  const mappedCurrent = {
...
const useOpenMeteo = (options: useOpenMeteoOptions) => {
  ...
  const temperatureUnit = unit?.temperature ?? 'celsius';
  const windSpeedUnit = unit?.wind_speed ?? 'kmh';

  let params: OpenMeteoParams = {
    latitude: lat,
    longitude: lon,
    daily:
      'temperature_2m_max,temperature_2m_min,weather_code,relative_humidity_2m_max,wind_speed_10m_max',
